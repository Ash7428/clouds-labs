...
2) использование прямой ссылки не рекомендуется, так как такие файлы нельзя удалить, и они увеличивают размер образа. 

3) При такой директории в контейнер копируется больше данных, чем нужно, а также это делается не очень удобно, указывая путь на папку в инструкции COPY.
